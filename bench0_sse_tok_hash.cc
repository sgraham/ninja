...
void LexIdentifier(Buffer& B, Token& T, const char* CurPtr) {
  ...
  //llvm::StringMapEntry<IdentifierInfo*> &Entry =
  //  HashTable.GetOrCreateValue(StringPiece(IdStart, T.length));
...
//if (g_count > 1) printf("%d\n", t.kind);
    if (t.kind == kIdentifier && t.ident[-1] == '\n') {
      ...
      if (t.info == kw_subninja || t.info == kw_include) {
        b.cur++;  // skip space
        LexPath(b, t, b.cur);
...
int main(int argc, const char* argv[]) {
  ...
  *s++ = '\0';
  ...
  kw_subninja = &Identifiers.get("subninja");
  kw_include = &Identifiers.get("include");
